...
const Home = () => {
  ...
  const fetchSubreddits = async () => {
    try {
      const response = await fetch(`${API_URL}/reddit/subreddits`);
      if (response.ok) {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/reddit/fetch/${searchTerm}`);
      if (response.ok) {
        const data = await response.json();
        if (data.message === "Veriler MongoDB'ye kaydedildi!") {
          const postsResponse = await fetch(`${API_URL}/reddit/posts/${searchTerm}`);
          if (postsResponse.ok) {
  ...
  const handleSubredditClick = async (subreddit) => {
    ...
    setCurrentPage(1);
    try {
      const response = await fetch(`${API_URL}/reddit/posts/${subreddit}`);
      if (response.ok) {
        const data = await response.json();
  ...
  const handleDeleteSubreddit = async (subreddit) => {
    try {
      ...
      if (response.ok) {
        ...
        setSubreddits(subreddits.filter((item) => item !== subreddit));
      } else {
