...
const Home = () => {
  ...
  const fetchSubreddits = async () => {
    try {
      const response = await fetch('https://redditbackend2.onrender.com:8080/api/reddit/subreddits');
      if (response.ok) {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(`https://redditbackend2.onrender.com:8080/api/reddit/fetch/${searchTerm}`);
      if (response.ok) {
        const data = await response.json();
        if (data.message === "Veriler MongoDB'ye kaydedildi!") {
          const postsResponse = await fetch(`https://redditbackend2.onrender.com:8080/api/reddit/posts/${searchTerm}`);
          if (postsResponse.ok) {
  ...
  const handleSubredditClick = async (subreddit) => {
    ...
    setCurrentPage(1);
    try {
      const response = await fetch(`https://redditbackend2.onrender.com:8080/api/reddit/posts/${subreddit}`);
      if (response.ok) {
        const data = await response.json();
